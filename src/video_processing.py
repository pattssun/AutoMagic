...
def create_image_clip_for_body(start_time, end_time, clip_size, image_path=None):
    ...
    # Add a zoom effect to the image from a faded smaller size to the full size non-faded
    image_clip = image_clip.resize(lambda t: 1 + 0.04 * t).crossfadein(image_clip.duration / 4)
